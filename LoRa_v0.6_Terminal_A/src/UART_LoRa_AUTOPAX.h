...
//------ CALLBACK FUNCTIONS IO -----------//
void UART_IO_rx_cb(struct usart_module *const usart_module);
void UART_IO_tx_cb(struct usart_module *const usart_module);

//------ DATA FUNCTIONS ----------------------//
void UART_SAM_To_RPi(uint8_t* data);		// UART between SAM R34 and RPi
void UART_SAM_To_IO(uint8_t* data);			// UART between SAM R34 and IO machine
